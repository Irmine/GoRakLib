...
func TestBinaryRead(t *testing.T) {
	...
	if ReadBigTriad				(&buf, &off) != 123456 { t.Error("Incorrect buffer reading: ReadBigEndianTriad")}
	if ReadBigTriad				(&buf, &off) != 0x1 { t.Error("Incorrect buffer reading: ReadBigEndianTriad")}
	if ReadLittleTriad			(&buf, &off) != 123456 { t.Error("Incorrect buffer reading: ReadLittleEndianTriad")}
	if ReadLittleTriad			(&buf, &off) != 0x1 { t.Error("Incorrect buffer reading: ReadLittleEndianTriad")}
}
